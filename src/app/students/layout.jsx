...
export default function Layout() {
  ...
  const [weeksData, setWeeksData] = useState(Array.from({ length: totalWeeks }, () =>
    weekdays.map(() => ({
      date: dayjs(),
      timeIn: '',
      timeOut: '',
  ...
  const handleFieldChange = (weekIndex, dayIndex, field, value) => {
    const newWeeksData = weeksData.map((week, wIndex) =>
      wIndex === weekIndex ? week.map((day, dIndex) =>
  ...
  const calculateTotalHours = (timeIn, timeOut) => {
    if (dayjs(timeIn, 'HH:mm').isValid() && dayjs(timeOut, 'HH:mm').isValid()) {
      const hours = dayjs(timeOut, 'HH:mm').diff(dayjs(timeIn, 'HH:mm'), 'hour', true);
                  ...
                  <Grid container spacing={2}>
                    <Grid item xs={3}>
                      <DatePicker
                        label="Date"
                        value={day.date}
                        onChange={(newValue) => handleFieldChange(selectedWeek, index, 'date', newValue)}
                    ...
                    <Grid item xs={3}>
                      <TextField
                        ...
                        type="time"
                        value={day.timeIn}
                        onChange={(e) => handleFieldChange(selectedWeek, index, 'timeIn', e.target.value)}
                        fullWidth
                      />
                    </Grid>
                    <Grid item xs={3}>
                      <TextField
                        ...
                        type="time"
                        value={day.timeOut}
                        onChange={(e) => handleFieldChange(selectedWeek, index, 'timeOut', e.target.value)}
                        fullWidth
                      />
                    </Grid>
                    <Grid item xs={3}>
                    ...
                    <Grid item xs={12}>
                      <TextField
                        ...
                        multiline
                        value={day.tasks}
                        onChange={(e) => handleFieldChange(selectedWeek, index, 'tasks', e.target.value)}
                        fullWidth
                        margin="normal"
                        rows={4}
                      />
                    ...
                    <Grid item xs={6}>
                      <TextField
                        ...
                        multiline
                        value={day.plan}
                        onChange={(e) => handleFieldChange(selectedWeek, index, 'plan', e.target.value)}
                        fullWidth
                        margin="normal"
                        rows={4}
                      />
                    ...
                    <Grid item xs={6}>
                      <TextField
                        ...
                        multiline
                        value={day.action}
                        onChange={(e) => handleFieldChange(selectedWeek, index, 'action', e.target.value)}
                        fullWidth
                        margin="normal"
                        rows={4}
                      />
